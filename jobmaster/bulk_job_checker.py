...
class BulkJobChecker:
    ...
    def auto_load_main_data(self):
        """Try to automatically load main data if default file exists"""
        if os.path.exists(config.DEFAULT_INPUT_FILE):
            self.load_main_data(config.DEFAULT_INPUT_FILE)
...
LE-0065-15-JULY-2025-01FAC"""
    ...
    def upload_job_file(self):
        ...
        file_path = filedialog.askopenfilename(
            ...
            initialdir=config.SAMPLES_DIR if os.path.exists(config.SAMPLES_DIR) else config.PROJECT_ROOT,
            filetypes=[
                ("Text files", "*.txt"),
                ("CSV files", "*.csv"),
                ("All files", "*.*")
